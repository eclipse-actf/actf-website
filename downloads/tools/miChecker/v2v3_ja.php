...
<div id="midcolumn">
	...
	本ページでは、v3.00およびv3.10への移行時に行われた評価ルールの変更について、v2からの差分を中心にその概要を説明します。<br>
...
　	<h3>固定サイズフォントが用いられていた場合の評価</h3>
	...
	<h3>"本文へ移動"などのスキップリンクおよびウェブページの構造化などに関する評価 (v3.10から)</h3>
	<p>
	以前のmiCheckerではウェブページ内に"本文へ移動"などのスキップリンクが提供されておらず、到達するまでに長い時間がかかるコンテンツが存在している場合に問題を報告していました。
	miChecker v3.1以降では、"本文へ移動"などのスキップリンクの存在に加えて、main要素などを用いた構造化の有無なども踏まえて指摘を細分化しています。
	新しいmiCheckerを用いて指摘事項の確認を行い、その指示に従ってください。
	</p>

	<h3>同じ name 属性値を持つtype="radio","checkbox"のinput要素のグループとfieldset要素に関する評価 (v3.10から)</h3>
	<p>
	以前のmiCheckerではウェブページ内で同じname属性の値をもつラジオボタンまたはチェックボックス（type="radio"または"checkbox"の input 要素）のグループが同一のfieldset要素に含まれていない場合にこの問題を報告していました。
	miChecker v3.1以降では、対象となるinput要素とfieldset要素との親子関係の組み合わせ（WAI-ARIAによるグルーピングを含む）を踏まえて指摘を細分化していますので、新しいmiCheckerを用いて指摘事項の確認を行い、その指示に従ってください。	
	</p>


	<h3>テキストによる代替に関する評価 (v3.10から)</h3>
	<p>
	miChecker v3.1以降では、画像に対する代替テキスト、入力項目に対するラベル、テーブルの名前などの評価に際して、aria-label, aria-labelledbyなどが用いられることを考慮した評価に変更しています。
	この結果、以前のmiCheckerで指摘されていた問題が指摘されなくなることがありますが、要判断箇所の指摘事項およびハイライト機能、対応するソースコードの確認機能などを用いて、提供されているテキストによる代替が正しいものか確認してください。
	テキストによる代替の提供に限らず、WAI-ARIAを用いる場合には、対象とするブラウザおよび支援技術の組み合わせで正しく意図した通りに動作していることを確認する必要がある点にも留意してください。
	<a href="https://validator.w3.org/">W3CのMarkup Validation Service</a>を用いると、WAI-ARIAではなくHTMLの標準的な属性等を用いるべき箇所を指摘される場合がありますので、あわせて確認するようにして下さい。
	</p>
