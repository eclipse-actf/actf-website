...
　	<h3>固定サイズフォントが用いられていた場合の評価</h3>
	...
	最新のブラウザでは問題とならない場合が一般的になっていますので、もしmiChecker（v2系）を利用して指摘を受けた場合は、対象ブラウザでの実際の挙動を確認して対応を行ってください
	（要判断箇所相当として取り扱ってください）。
	</p>
	...
	<p>(フォントサイズの指定されている箇所(style属性かCSS内かなど)によって指摘の区分が変わります)</p>
	...
	(miChecker v3では、ロービジョンモードにおける固定サイズフォント利用に関する指摘も、音声ユーザビリティモードと同様に要判断箇所に変更となっています)

	<hr>
	
	<h3>複雑なテーブルのth要素とscope属性に関する評価</h3>
	<p>
	複雑なテーブル(先頭行のみもしくは先頭列のみが全てth要素となっている様な単純なテーブルでない場合)におけるth要素とscope属性に関して指摘を行う際のメッセージを詳細化しています。
	...
	<p>
	長い代替テキストが用いられていた場合に、廃止となったlongdesc属性ではなくaria-describedby属性などの利用を促すよう指摘を変更しています。
	</p>
	<h4>v2での指摘事項</h4>
	<ul>
